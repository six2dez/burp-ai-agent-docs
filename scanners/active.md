...
![Screenshot: Active scanner settings](../assets/screenshots/active-scanner.png)
...
| Method | Description |
| :--- | :--- |
| **ERROR_BASED** | Look for database/framework error messages in responses. |
| **BLIND_BOOLEAN** | Compare response differences when injecting true/false conditions. |
| **BLIND_TIME** | Measure response time delays (e.g., `sleep(5)` causing 5s delay). |
| **REFLECTION** | Check if the payload is reflected in the response body. |
| **OUT_OF_BAND** | DNS/HTTP callback detection via Burp Collaborator. |
| **CONTENT_BASED** | Check for specific content patterns indicating successful exploitation. |
