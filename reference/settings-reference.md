...
The Settings tabs use a shared internal design system with consistent layout, collapsible scanner sections, and automatic Burp light/dark re-theming. The v0.8.0 redesign preserved the settings that existed at that point; later schema versions added prompt-library state, encrypted secrets, and external MCP servers. The tables below describe the current main branch.
...
| ----------------------------------- | ------------------ | ------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Preferred Backend**               | Dropdown           | `burp-ai`                                                     | Backend used for new chat sessions. All registered backends are shown regardless of CLI availability; errors are reported at usage time via health check. The persisted default is the built-in Burp AI backend; select another backend explicitly when Burp does not expose AI capability. |
...
| **Ollama CLI Command**              | Text               | `ollama run llama3.1`                                        | Command used for CLI validation and as a fallback source for the HTTP model name.                                                                                                                                                                                                  |
| **Ollama URL**                      | Text               | `http://127.0.0.1:11434`                                      | Base URL for Ollama API.                                                                                                                                                                                                                                                           |
| **Ollama Model**                    | Text               | `llama3.1`                                                    | Model identifier sent to Ollama.                                                                                                                                                                                                                                                   |
| **Ollama API Key**                  | Password           | _(empty)_                                                     | Optional bearer token for Ollama-compatible servers; encrypted at rest in Burp preferences.                                                                                                                                                                                        |
...
| **Ollama Context Window**           | Persisted/internal | `8192`                                                        | Context window size (persisted in range `2048-128000`; loaded values up to `256000` are accepted). The redesigned panel does not currently expose a control for it, but preserves the loaded value.                                                                                  |
...
| **LM Studio API Key**               | Password           | _(empty)_                                                     | Optional bearer token for OpenAI-compatible servers; encrypted at rest in Burp preferences.                                                                                                                                                                                        |
...
| **OpenAI-Compatible API Key**       | Password           | _(empty)_                                                     | Optional bearer token; encrypted at rest in Burp preferences.                                                                                                                                                                                                                      |
...
| **NVIDIA NIM API Key**              | Password           | _(empty)_                                                     | Bearer token for NVIDIA NIM; encrypted at rest in Burp preferences.                                                                                                                                                                                                                |
| **NVIDIA NIM Extra Headers**        | Multiline          | _(empty)_                                                     | Extra headers, one per line (`Header: value`).                                                                                                                                                                                                                                     |
| **NVIDIA NIM Timeout**              | Number             | `120`                                                         | Timeout in seconds (range: 30-3600).                                                                                                                                                                                                                                               |
...
| **Perplexity API Key**              | Password           | _(empty)_                                                     | `pplx-…` bearer token; encrypted at rest in Burp preferences.                                                                                                                                                                                                                      |
| **Perplexity Extra Headers**        | Multiline          | _(empty)_                                                     | Extra headers, one per line (`Header: value`).                                                                                                                                                                                                                                     |
| **Perplexity Timeout**              | Number             | `120`                                                         | Timeout in seconds (range: 30-3600).                                                                                                                                                                                                                                               |
| **Anthropic Model**                 | Text               | `claude-sonnet-4-6`                                          | Free-form Anthropic Messages API model identifier.                                                                                                                                                                                                                                |
| **Anthropic API Key**               | Password           | _(empty)_                                                     | Anthropic `x-api-key`; encrypted at rest in Burp preferences.                                                                                                                                                                                                                      |
...
| Setting                     | Type     | Default            | Description                                                                                                               |
| --------------------------- | -------- | ------------------ | ------------------------------------------------------------------------------------------------------------------------- |
| **Privacy Mode**            | Dropdown | `BALANCED`         | Selects built-in redaction policy (`STRICT`, `BALANCED`, `OFF`). Existing scanner findings keep the values they were built with; re-scanning does not rewrite them. |
| **Determinism Mode**        | Toggle   | Off                | Stabilizes covered ordering and host-pseudonym decisions so captured prompt bundles are easier to compare; it does not make model output deterministic or turn the bundle into a complete wire transcript. |
| **Host Anonymization Salt** | Persisted/internal | _(auto-generated)_ | Plaintext preference used as the RFC 5869 HKDF/HMAC-SHA256 input for stable `host-<12 hex>.local` pseudonyms in STRICT mode. The UI exposes **Rotate anonymization salt**, not the value itself. |
| **Custom Redaction Patterns** | Multiline | _(empty)_        | One regular expression per line. Validated on save and applied in every mode, including `OFF`; stored as plaintext configuration. |
...
| **AI Logger Max Entries**   | Number   | `500`              | Maximum entries in the in-memory logger buffer; persistence clamps the value to `50-5000`.                               |

The Privacy Mode tooltip in the current UI states the write-time boundary explicitly: scanner findings already recorded keep the values they were built with, and re-scanning does not rewrite them. See [Privacy Modes](../privacy/privacy-modes.md) for carrier coverage and residuals.
...
| **Allowed Origins**               | Multiline | _(empty)_                                 | Comma-, semicolon-, or newline-separated external CORS origins. In external mode, an empty list enables permissive CORS (`anyHost()`); bearer authentication still applies to ordinary non-health routes, while `POST /__mcp/shutdown` also accepts the bounded takeover proof. Production deployments should set an explicit allowlist. |
| **TLS Enabled**                   | Toggle    | Off                                       | Enable HTTPS for MCP server. It is forced on and cannot be disabled while External Access is enabled.                                             |
...
| **Keystore Password**             | Password  | _(auto-generated)_                        | Password used by the keystore; encrypted at rest under preferences key `mcp.tls.keystore.password`.                                             |
| **Max Concurrent Requests**       | Number    | `4`                                       | Max parallel MCP tool calls (range: 1-64).                                                                                                        |
| **Max body size (KB)** | Number | `2048` | Byte cap for the final serialized MCP tool result (range: 32-102400). History/site-map entries are also individually truncated to this cap before the bounded join. |
| **Scan Task TTL (min)**           | Persisted/internal | `120`                              | Retention for completed scan task references surfaced via MCP (range: 5-1440). Not currently exposed by the redesigned panel; its loaded value is preserved on save. |
| **Collaborator Client TTL (min)** | Persisted/internal | `60`                               | Retention for Collaborator secret keys allocated by MCP (range: 5-1440). Not currently exposed by the redesigned panel; its loaded value is preserved on save.   |
| **Enable Unsafe Tools**           | Toggle    | Off                                       | Master switch for tools marked unsafe.                                                                                                            |

### External MCP Servers

The **External Servers** sub-tab stores a list of outbound MCP connections. The default is an empty list. Each entry selects SSE or stdio transport, server name, URL or command, enabled state, and optional bearer token. Tokens are encrypted per field before the v5 settings blob is persisted; stdio remains an explicit opt-in because it launches a local process. See [External MCP Servers](../mcp/external-servers.md).
...
| **Persistent max (MB)**       | Number   | `50`                                                                                                                      | Maximum disk space for persistent cache in MB (range: 10-500). Once usage exceeds the maximum, oldest files are removed until usage is at or below 80% of the configured cap. |
| **Warn threshold (tokens)**   | Number   | `0` (off)                                                                                                                 | Show a chat banner when cumulative token usage for the current Burp process reaches the threshold.         |
| **Hard cap (tokens)**         | Number   | `0` (off)                                                                                                                 | Pause passive scanning when cumulative token usage reaches the cap; chat remains usable.                  |
...
The same **Custom Prompts** tab contains the BountyPrompt controls:

| Setting                         | Type            | Default                        | Description                                                                     |
| ------------------------------- | --------------- | ------------------------------ | ------------------------------------------------------------------------------- |
| **Enabled**                     | Toggle          | Off                            | Enables the curated BountyPrompt submenu; when off, the submenu remains visible but disabled. |
| **Prompts directory**           | Text            | `~/Tools/BountyPrompt/prompts` | Directory containing BountyPrompt JSON files.                                   |
| **Auto-create issues**          | Toggle          | On                             | Auto-create Burp issues for parsed findings from `outputType = issue` prompts.  |
| **Issue threshold**             | Number          | `90`                           | Minimum parsed confidence (0-100) required for issue creation.                  |
| **Enabled prompt IDs**          | Multi-line text | Curated IDs                    | Allowlist of prompt IDs, comma- or newline-separated. An empty value falls back to the default curated set. |
...
| `~/.burp-ai-agent/contexts/`      | Reserved directory created by `AuditLogger`; no current production caller writes standalone context files. |
...
* **CLI process timeout**: `120s` overall hard ceiling (internally clamped to `30-3600s`; the current settings panel does not expose a timeout control for CLI backends).
* **HTTP backend retry schedule**: up to `6` total attempts for classified transient transport exceptions. The current loop can wait before five retries: `500 / 1000 / 1500 / 2000 / 3000 ms`. HTTP error responses are not retried in the same call.
