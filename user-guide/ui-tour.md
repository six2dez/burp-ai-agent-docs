...
* Markdown-rendered responses delivered as a completed block by current built-in backends (the renderer supports chunks, but the current adapters invoke it once per response).
...
The panel is docked at the bottom of the **Custom AI Agent** Suite tab and can be resized/collapsed.
...
| **Prompt Templates** | Built-in request and scanner-issue template editing. |
| **Custom Prompts** | Saved free-form prompt library plus BountyPrompt enablement, directory, issue threshold, and curated-ID controls. |
...
Every right-click action that auto-captures context (proxy item, scanner issue, site-map node, Repeater) opens a modal before that captured context is dispatched. The modal shows the action, current privacy mode, resolved action prompt, and post-policy context JSON. Backend protocol framing, conversation history, a tool preamble, and a separate system-role profile can be added afterward; see [Context Menus → Context Preview Dialog](context-menus.md#context-preview-dialog). Confirm with **Send** or abort with **Cancel**.
